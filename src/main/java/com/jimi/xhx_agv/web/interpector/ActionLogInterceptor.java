...
import com.jimi.xhx_agv.constant.ResultCode;
import com.jimi.xhx_agv.entity.model.ActionLog;
import com.jimi.xhx_agv.entity.model.User;
import com.jimi.xhx_agv.util.IPUtil;
...
public class ActionLogInterceptor implements Interceptor {
	...
	public void intercept(Invocation invocation) {
		...
		if(log != null) {
			...
			String token = controller.getPara(TokenBox.TOKEN_ID_KEY_NAME);
			if (token != null) {
				Integer userId = TokenBox.get(token, "userId");
				if (userId != null) {
					User user = User.dao.findById(userId);
					String nickname = user.getUser();
					actionLog.setUserName(nickname);
				}
			}
			//执行
